...
const METHODS = [
  { key: 'cash', label: 'Cash', icon: '≡ƒÆ╡' },
  { key: 'upi', label: 'UPI / GPay', icon: '≡ƒô▒' },
  { key: 'card', label: 'Card Payment', icon: '≡ƒÆ│' },
  { key: 'other', label: 'Other', icon: '≡ƒöä' }
]
      ...
      <div className="flex items-center gap-3 bg-white p-3 rounded-2xl border border-slate-100 shadow-sm">
        <div className="search-wrap">
          ...
          <input
            ...
            placeholder="Search by patient nameΓÇª"
            value={search}
        ...
        footer={
          <div className="flex gap-3">
            ...
            <button id="btn-save-bill" onClick={handleCreate} disabled={saving || !selPatient || billItems.length === 0} className="btn-primary">
              {saving ? 'SavingΓÇª' : 'Generate Invoice Γ£ô'}
            </button>
          </div>
        }
      >
        <div className="space-y-5 text-slate-800">
          ...
          {selPatient ? (
            <div className="flex items-center justify-between bg-teal-50/50 border border-teal-100 rounded-2xl px-5 py-3">
              <div>
                ...
                <p className="text-xs text-teal-600 font-medium">≡ƒô₧ {selPatient.phone || 'No phone number'}</p>
              </div>
              <button
          ...
          {selPatient && patAppts.length > 0 && (
            <div>
              ...
              >
                <option value="">ΓÇö Standalone Bill (No Appointment) ΓÇö</option>
                {patAppts.map(a => (
            ...
            <div className="bg-slate-50 rounded-2xl p-5 border border-slate-150 space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-12 gap-3">
                <div className="sm:col-span-5">
                  ...
                  >
                    <option value="">ΓÇö Select Predefined Treatment ΓÇö</option>
                    {PREDEFINED_TREATMENTS.map(t => (
                      <option key={t.name} value={t.name}>{t.name} (Γé╣{t.price})</option>
                    ))}
                  </select>
                </div>

                <div className="sm:col-span-2">
                  <label className="text-[11px] font-bold uppercase text-slate-500 block mb-1">Charges (Γé╣)</label>
                  <input
                    type="number"
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {billItems.map((item, idx) => (
                        <tr key={idx} className="hover:bg-slate-50/50">
                          ...
                          <td className="px-3 py-2 font-mono">{item.tooth_number || 'ΓÇö'}</td>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 border-t border-slate-100 pt-4">
                ...
                <div>
                  ...
                  />
                  <span className="text-[10px] text-slate-400 mt-1 block">Amount Paid Now (Γé╣)</span>
                </div>
              </div>
              <div>
        ...
        footer={
          <div className="flex gap-2">
            ...
            <button id="btn-confirm-pay" onClick={handlePayment} disabled={saving} className="btn-primary">
              {saving ? 'SavingΓÇª' : 'Record Payment Γ£ô'}
            </button>
          </div>
        }
      >
        {activeBill && (
          <div className="space-y-4 text-slate-800">
            ...
            <div>
              <label className="label">Paid Amount (Γé╣)</label>
              <input
            ...
            {historyItems.map((item, idx) => (
              <div key={item.id || idx} className="flex justify-between items-center bg-slate-50 p-4 rounded-xl border border-slate-100">
                <div>
                  ...
                  <p className="text-xs text-slate-500 font-medium capitalize mt-0.5">{item.method} ΓÇó {new Date(item.paid_at).toLocaleString()}</p>
                </div>
        ...
        footer={
          <div className="flex gap-2 w-full">
            ...
            <button onClick={handleEmail} disabled={saving} className="btn-primary flex-1">
              {saving ? 'SendingΓÇª' : 'Send Email Γ£ë'}
            </button>
          </div>
        }
      >
...
function generateReceiptHTML(bill, treatments = [], settings) {
  ...
  const cur = settings?.currency || 'Γé╣'
...
<head>
  <meta charset="utf-8">
  <title>Invoice ΓÇô ${bill.patient_name}</title>
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* ΓöÇΓöÇ Patient Info Block ΓöÇΓöÇ */
    .patient-info-grid {
      display: grid;
...
<body>
  ...
  <div class="wrap">
    ...
    <p class="status-line">${isPaid ? 'Γ£ô Fully Settled' : 'ΓÜá Balance Due'}</p>
    ...
    <p class="ty">Thank you for choosing ${settings?.clinic_name || "Dr. Mahe's Dentistry"} ΓÇö Wishing you a healthy smile.</p>
